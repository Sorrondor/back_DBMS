...
FROM TBL_PRODUCT
GROUP BY ID, PRODUCT_PRICE
HAVING PRODUCT_PRICE < (
...
-- 평균 재고보다 낮은 상품
-- WHERE절 서브쿼리
SELECT ID, PRODUCT_NAME, PRODUCT_STOCK
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK < (
...
-- 댓글을 단 사용자의 거주지 조회, ID와 ADDRESS 조회
SELECT USER_ADDRESS 
FROM TBL_USER
WHERE ID IN (
	SELECT USER_ID
...
SELECT REPLY_CONTENT AS "제목에 맥북을 포함하고 있는 게시글에 달린 댓글"
FROM TBL_REPLY
WHERE POST_ID = (
	SELECT ID
	FROM TBL_POST
...
SELECT USER_EMAIL AS "내용에 컴퓨터를 포함하고 있는 글을 작성한 유저의 이메일"
FROM TBL_USER
WHERE ID IN (
	SELECT USER_ID
	FROM TBL_POST
...
-- 5) 가장 나이가 어린 사용자가 작성한 게시글들 조회, ROWNUM 못 풀었음
SELECT *
FROM TBL_POST
WHERE USER_ID IN (
	SELECT ID
...
-- 5) 가장 나이가 어린 사용자가 작성한 게시글들 조회, ROWNUM 정답
SELECT * 
FROM TBL_POST
WHERE USER_ID IN (
   SELECT ID
