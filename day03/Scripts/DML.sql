...
INSERT INTO TBL_FIELD_TRIP
VALUES(SEQ_FIELD_TRIP.NEXTVAL, '아이스크림 빨리먹기 대회', '아이스크림 누가 잘 먹나', 100);

INSERT INTO TBL_FIELD_TRIP
VALUES(SEQ_FIELD_TRIP.NEXTVAL, '고구마 심기', '고구마 왕 커요', 10);
...
INSERT INTO TBL_FIELD_TRIP
VALUES(SEQ_FIELD_TRIP.NEXTVAL, '치즈 제작하기', '여기 치즈 저기 치즈', 5);
...
SELECT FIELD_TRIP_NUMBER AS "체험 글자를 포함한 체험의 총 인원"
FROM TBL_FIELD_TRIP
...
SELECT AVG(FIELD_TRIP_NUMBER) AS "제목에 '고구마' 또는 '아이스크림' 들어간 체험 학습의 평균 인원수"
FROM TBL_FIELD_TRIP
WHERE FIELD_TRIP_TITLE LIKE '%고구마%'
OR FIELD_TRIP_TITLE LIKE '%아이스크림%'; 

-- 체험학습 제목이 '기'로 끝나는 체험학습의 최소 인원수
SELECT MIN(FIELD_TRIP_NUMBER) AS "인원수"
FROM TBL_FIELD_TRIP
...
SELECT
	PRODUCT_PRICE || '개',
	SUM(PRODUCT_STOCK) || '개' AS "상품 개수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE;

-- 2) 재고가 50개 이상인 상품의 개수
SELECT COUNT(ID) AS "재고가 50개 이상인 상품의 개수"
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK >= 50;

-- 3) 재고가 30개 미만인 상품의 평균 가격
SELECT AVG(PRODUCT_PRICE) AS "재고가 30개 미만인 상품의 평균 가격"
FROM TBL_PRODUCT
...
SELECT
	PRODUCT_NAME,
	PRODUCT_STOCK AS "재고",
	SUM(PRODUCT_STOCK) AS "재고별 개수",
	ROUND(AVG(PRODUCT_PRICE)) AS "재고별 평균가격"
FROM TBL_PRODUCT
...
-- 모든 상품의 재고와 가격의 합
SELECT
	PRODUCT_STOCK + PRODUCT_PRICE AS "상품의 재고와 가격의 합"
FROM TBL_PRODUCT;

-- 상품 가격대별 기대수익
SELECT
	PRODUCT_PRICE AS "상품 가격대",
	PRODUCT_PRICE * SUM(PRODUCT_STOCK) AS "상품 기대수익"
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE;

-- 가격대별 10000원으로 살 수 있는 배승원의 노트북 개수
SELECT
	PRODUCT_PRICE || '원' AS "가격대",
	FLOOR(10000 / PRODUCT_PRICE) || '개' AS "10000원으로 살 수 있는 배승원의 노트북 개수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE
